...
class Tool(object):
    ...
    ./stackcount -U t:sched:sched_switch   # user stacks only
        ...
        self.probe = Probe(self.args.pattern, self.kernel_stack,
                           self.user_stack, self.args.regexp, self.args.pid,
                           self.args.perpid, self.args.cpu)
    ...
    def _print_uframe(self, addr, pid):
        ...
        if self.args.offset:
            print("%s" %
                  self.probe.bpf.sym(addr, pid, show_offset=True).decode())
        else:
